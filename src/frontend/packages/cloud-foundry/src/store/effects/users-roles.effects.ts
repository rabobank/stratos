...
import { ManageUsersSetUsernamesHelper } from '../../features/cf/users/manage-users/manage-users-set-usernames/manage-users-set-usernames.component';
...
export class UsersRolesEffects {
      ...
      mergeMap((action) => {
        ...
        action.changedRoles.forEach((change) => {
          const apiAction: ICFAction = {
            ...
            entityType: change.spaceGuid
              ? spaceEntityType
              : organizationEntityType,
            updatingKey: ChangeCfUserRole.generateUpdatingKey(
              change.role,
              change.userGuid
            ),
            options: null,
  ...
  executeUsersRolesChange$ = createEffect(() =>
    this.actions$.pipe(
    ...
    if (orgUserChanges.length) {
      ...
      if (orgUserChanges[0].add) {
        ...
        return this.executeChanges(
          ...
          cfSession,
          orgUserChanges,
          action.setByUsername,
          usersRoles.usernameOrigin
        ).pipe(
  ...
  private executeChanges(
    ...
    changes.forEach((change) => {
      ...
      const action = this.createAction(
        ...
        change,
        setByUsername
          ? ManageUsersSetUsernamesHelper.usernameFromGuid(change.userGuid)
          : null,
        usernameOrigin
      );
    ...
    const allObservables =
      observables.length === 0
        ? observableOf([true])
        : observableCombineLatest(...observables);
    return allObservables.pipe(first());
  }
  ...
  private createActionObs(action: ChangeCfUserRole): Observable<any> {
    return entityCatalog
      .getEntity(action)
      .store.getEntityMonitor(action.guid)
