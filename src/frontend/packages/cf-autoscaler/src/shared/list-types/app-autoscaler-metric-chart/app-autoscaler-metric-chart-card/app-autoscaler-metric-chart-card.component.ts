...
export class AppAutoscalerMetricChartCardComponent
  extends CardCell<APIResource<AppScalingTrigger>>
  implements IListRowCell {
  static columns = 1;
  listData: {
    label: string;
  ...
  set row(row: APIResource<AppScalingTrigger>) {
    ...
    if (row) {
      if (row.entity.query && row.entity.query.params) {
        ...
        this.metricType = AutoscalerConstants.getMetricFromMetricId(
          row.metadata.guid
        );
        this.metricData$ = this.getAppMetric(
          this.metricType,
          row.entity,
          this.paramsMetrics
        );
  ...
  getAppMetric(
    ...
    params: AutoscalerPaginationParams
  ): Observable<AppAutoscalerMetricData[]> {
    const action = new GetAppAutoscalerAppMetricAction(
    ...
    return getPaginationObservables<AppAutoscalerMetricData>(
      ...
      true
    ).entities$.pipe(filter((entities) => !!entities));
  }
}
